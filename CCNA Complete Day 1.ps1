...
!!!DAPAT 7 UNDER61SCORE ANG NA EDIT MO  ( LINE 34 to 68 PASTE TO COREBABA SWITCH )
!@@@LeafSwitchONLY
config t
   hostname COREbaba-61
   enable secret pass
   service password-encryption
   no logging console
   no ip domain-lookup
...
config t
   ...
   Int Gi 0/0/1
	 ...
	 ip add 200.0.0.61 255.255.255.0
	 no shutdown
   Int Loopback 0
     description VIRTUALIP-FOR-ROUTING
	 ip add 61.0.0.1 255.255.255.255
 end
...
config t
router ospf 1
router-id 10.61.61.4
network 10.61.0.0  0.0.255.255 area 0
int gi 0/1
 ip ospf network point-to-point
end
...
conf t
 ...
 NO ip route 10.61.0.0 255.255.0.0 200.0.0.61
...
dial-peer voice 69 voip
 ...
 destination-pattern 6169
 session target ipv4:10.61.100.8
 incoming called-number 6169
 dtmf-relay h245-alphanumeric
 codec g711ulaw
 no vad
!
telephony-service
 moh "flash:/en61bacd61music61on61hold.au"
!
application
 service rivanaa flash:app-b-acd-aa-3.0.0.2.tcl
  ...
  param welcome-prompt flash:en61bacd61welcome.au
...
http://docwiki.cisco.com/wiki/Cisco61IOS61Voice61Troubleshooting61and61Monitoring61--61Tcl61IVR61Troubleshooting
...
!
 voice register global
  mode cme
  source-address 10.61.100.8 port 5060
  max-dn 12
  max-pool 12
...
!
  voice register pool 1
    id mac c0ed.e5e7.c35f
    number 1 dn 1
    dtmf-relay sip-notify
    username 6161 password 6161
    codec g711ulaw
!
  voice register pool 2
    id mac 405e.f6be.6a05
    number 1 dn 2
    dtmf-relay sip-notify
    username 6252 password 6252
    codec g711ulaw
!
